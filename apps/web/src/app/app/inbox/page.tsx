...
type Search = { mailbox?: string; folder?: string; email?: string };
...
export default async function InboxPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  try {
    ...
    if (sp.email) openEmail = await jmap.getEmail(sp.email);
  } catch (e) {
    error =
    ...
    <Shell>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {folders.map((f) => (
          <Link
            key={f.id}
            href={q({ folder: f.id, email: undefined })}
